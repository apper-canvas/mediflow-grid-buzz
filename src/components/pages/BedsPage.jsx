...
const getPatientName = (patientId) => {
    const patient = patients.find(p => p.id_c === patientId);
    return patient ? patient.name_c || patient.Name : "Unknown Patient";
  };

const getAvailablePatients = () => {
    return patients.filter(p => p.status_c !== "Discharged" && !p.current_ward_c);
  };

const filteredBeds = beds.filter(bed => {
    if (selectedWard === "All") return true;
    return bed.ward_c === selectedWard;
  });

const wardStats = wards.slice(1).map(ward => {
    const wardBeds = beds.filter(b => b.ward_c === ward);
    const occupied = wardBeds.filter(b => b.status_c === "Occupied").length;
    const available = wardBeds.filter(b => b.status_c === "Available").length;
    const maintenance = wardBeds.filter(b => b.status_c === "Maintenance").length;
...
<ApperIcon name={getBedIcon(bed.type_c)} size={20} className="text-primary" />
                      <span className="font-semibold text-gray-900">{bed.ward_c} - {bed.number_c}</span>
                    </div>
<Badge variant={getBedStatusColor(bed.status_c)}>
                      {bed.status_c}
                    </Badge>
                  </div>

<div className="space-y-2 text-sm text-gray-600 mb-4">
                    <p>Type: <span className="font-medium">{bed.type_c}</span></p>
                    {bed.patient_id_c && (
                      <p>Patient: <span className="font-medium">{getPatientName(bed.patient_id_c)}</span></p>
                    )}
<p>Last Cleaned: <span className="font-medium">
                      {new Date(bed.last_cleaned_c).toLocaleDateString()}
                    </span></p>
                  </div>
...
<div className="space-y-2 max-h-60 overflow-y-auto">
                  {getAvailablePatients().map((patient) => (
                    <button
                      key={patient.Id}
                      onClick={() => handleAssignPatient(selectedBed.Id, patient.id_c)}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={patient.status_c.toLowerCase()}>
                          {patient.status_c}
                        </Badge>
                      </div>
                    </button>
