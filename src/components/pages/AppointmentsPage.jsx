...
const filteredAppointments = appointments.filter(appointment => {
    if (selectedDepartment !== "All" && appointment.department_c !== selectedDepartment) {
      return false;
    }
    return true;
  });
...
const getAppointmentsForDate = (date) => {
    return filteredAppointments.filter(appointment =>
      isSameDay(parseISO(appointment.date_time_c), date)
    );
  };
...
value={newAppointment.duration_c}
              ...
              <div className="space-y-2">
                <Label>Notes</Label>
                <textarea
value={newAppointment.notes_c}
                  onChange={(e) => setNewAppointment({...newAppointment, notes_c: e.target.value})}
...
<div>
                        <p className="font-semibold text-gray-900">{appointment.type_c}</p>
                        <p className="text-sm text-gray-600">{appointment.department_c}</p>
                        <p className="text-sm text-gray-500">
                          Patient: {appointment.patient_id_c} | Doctor: {appointment.doctor_id_c}
                        </p>
<p className="text-xs text-gray-400">
                          {format(parseISO(appointment.date_time_c), "PPP p")} ({appointment.duration_c} min)
                        </p>
                      </div>
                    </div>
...
<p className="font-medium">
                            {format(parseISO(appointment.date_time_c), "HH:mm")}
                          </p>
                          <p className="truncate">{appointment.type_c}</p>
                          <p className="truncate opacity-80">{appointment.department_c}</p>
                        </div>
                      ))}
                    </div>
                  </div>
